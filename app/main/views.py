...
#为什么减去1?
    ...
    pagination = post.comments.order_by(Comment.timestamp.asc()).paginate(page, \
                                                    per_page=current_app.config['FLASK_COMMENTS_PER_PAGE'], \
                                                                    error_out=False)
...
def followed_by(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page', 1, type=int)
    pagination = user.followed.paginate(page, per_page=current_app.config['FLASK_FOLLOWERS_PER_PAGE'], 
                                        error_out=False)
    follows = [{'user': item.followed, 'timestamp': item.timestamp} for item in pagination.items]
    logging.debug(follows)
